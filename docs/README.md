...
2. **Дополнения форка** — то, чего нет у апстрима:
   - **реестр отличий форка → [`ru/fork-changes.md`](ru/fork-changes.md)** —
     главный документ: все наши правки, файлы, риск конфликта при мерже, статус
     PR. Ведётся обязательно при любом изменении кода (правило в `../CLAUDE.md`);
   - деплой холдинга → [`ru/deploy.md`](ru/deploy.md) (автозапуск systemd,
     порт 3302, Tailscale).
