...
### 4.12 Микрофон — в правом верхнем углу поля ввода (мобильный)

- **Слой:** UI · **Столкновение:** 🟡 (общий upstream-композер) · **PR:** можно
  предлагать.
- **Что:** ниже брейкпоинта `sm` кнопка микрофона вынесена из нижней панели и
  закреплена в правом верхнем углу textarea — круглая, 36×36, как в оригинальном
  Claude Code. Выше `sm` всё как у апстрима: микрофон в нижнем ряду.
- **Зачем:** диктовка — основной способ ввода на телефоне (по словам владельца
  форка — «в 99% случаев»), а микрофон был одной из пяти мелких иконок в нижнем
  ряду. Плюс это освободило слот и вернуло панель в один ряд: после 1.37.3 она
  снова разъезжалась на два яруса даже с иконкой вместо названия модели (4.11).
- **Как:** у `VoiceInputButton` появился проп `variant`: `toolbar` (умолчание,
  прежнее поведение) и `floating`. Компонент презентационный — состояние записи
  живёт в композере, поэтому перенос не затронул логику.
- **Детали, которые легко сломать:**
  - у textarea на мобильном `pr-14` — иначе текст заезжает под кнопку;
  - плавающая и панельная кнопки взаимоисключающие (`sm:hidden` / `hidden
    sm:inline-flex`), иначе на планшете было бы два микрофона;
  - плашка ошибки у плавающего варианта раскрывается вниз (`top-full`), а не
    вверх — сверху её обрезал бы край композера.
- **Файлы:** `src/modules/chat/composer/VoiceInputButton.tsx`,
  `src/modules/chat/composer/ChatComposer.tsx`.
- **Проверено в браузере (360px):** панель в один ряд; кнопка 36×36 в углу,
  отступ текста 56px — длинный текст обтекает её и не заезжает; полный цикл
  записи (старт → красная кнопка «стоп» → возврат) отрабатывает; на 1280px
  композер не изменился.
