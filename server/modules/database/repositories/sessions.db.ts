    ...
    if (existing) {
      db.prepare(
        `UPDATE sessions SET
           ...
           project_path = ?,
           jsonl_path = ?,
           -- Two rules, ours first: a tool-generated session stays archived no
           -- matter what, because the plugin appends to it constantly and it
           -- would otherwise resurface in the sidebar on every write. Failing
           -- that, upstream's rule — un-archive only when the caller reports
           -- newer activity, so a rescan does not undo a manual archive.
           isArchived = CASE
             WHEN ? = 1 THEN 1
             WHEN ? IS NULL OR julianday(?) > julianday(updated_at) THEN 0
             ELSE isArchived
           END,
           custom_name = CASE
    ...
    db.prepare(
      `INSERT INTO sessions (session_id, provider, provider_session_id, custom_name, project_path, jsonl_path, isArchived, created_at, updated_at)
       ...
       ON CONFLICT(session_id) DO UPDATE SET
         ...
         -- Same two rules as the UPDATE above: an automated session stays
         -- archived (the plugin appends to it constantly, which would drag
         -- every review back into the sidebar), otherwise un-archive only on
         -- genuinely newer activity so a rescan cannot undo a manual archive.
         isArchived = CASE
           WHEN excluded.isArchived = 1 THEN 1
           WHEN ? IS NULL OR julianday(excluded.updated_at) > julianday(sessions.updated_at) THEN 0
           ELSE sessions.isArchived
         END,
         custom_name = CASE
