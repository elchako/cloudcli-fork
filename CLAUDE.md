...
6. **Читаемый список сеансов** — активный сеанс выделен явно (акцентная полоса,
   ...
   `CLOUDCLI_SESSION_TITLES=off`. Переименование (авто и ручное) сразу
   рассылается по WebSocket (`fullTitle` включён в `session_upserted`), поэтому
   шапка открытого чата, title вкладки и tooltip обновляются без перезагрузки;
   push-уведомления берут имя из БД на момент отправки, а не снапшот старта
   рана.
...
9. **Закрепление чатов** — пункт «Закрепить» в меню сеанса: закреплённые уходят
   наверх и не теряются. Работает и в «Разговорах», и внутри проекта (флаг на
   самой сессии — `sessions.isPinned`, поэтому у каждого проекта пины свои).
   Пин не считается активностью и не перебивает архив.
...
12. **Мобильная шапка в одну строку** — ниже `sm` вкладки только иконками
   (подпись = тултип/aria-label), заголовок чата и лента вкладок в одной
   строке (потолок ленты 55%, дальше её скролл). Служебные сеансы
   security-review дополнительно вычищаются sweep'ом при каждом старте
   (`archiveAutomatedSessions`) — закрывает сеансы, проиндексированные до
   появления автоархива; детектор читает транскрипт потоково, не целиком.
