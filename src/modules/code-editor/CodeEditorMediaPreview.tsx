...
export default function CodeEditorMediaPreview({
  ...
  useEffect(() => {
    ...
    // A project-scoped read goes through `api.readFileBlob`; the fallback is a
    // plain URL, so each candidate carries whichever of the two it needs.
    const candidates: ({ projectId: string; url?: undefined } | { projectId?: undefined; url: string })[] = [
      ...(projectId ? [{ projectId }] : []),
      ...(isAbsolutePath
        ? [{ url: `/api/assets/local-media?path=${encodeURIComponent(file.path)}` }]
        : []),
    ];
    ...
    const loadMedia = async () => {
      try {
        ...
        // Try each candidate in turn: the project endpoint first, then the
        // local-media fallback for paths outside the project root.
        let response: Response | null = null;
        for (const candidate of candidates) {
          const attempt = candidate.projectId !== undefined
            ? await api.readFileBlob(candidate.projectId, file.path, { signal: controller.signal })
            : await authenticatedFetch(candidate.url, { signal: controller.signal });
          if (attempt.ok) {
            response = attempt;
            break;
          }
