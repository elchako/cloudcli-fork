...
import { readVoiceConfig, revealVoiceApiKey, voiceConfigHeaders } from '@/shared/voiceConfig';
...
export const api = {
  ...
  // Kept flat rather than under `settings` because every call site predates
  // that grouping; the endpoints themselves live under /api/settings.
  getUserSettings: () => get('/api/settings/user-settings'),
  updateUserSettings: (settings: unknown, { merge = true }: { merge?: boolean } = {}) =>
    put(`/api/settings/user-settings${merge ? '' : '?merge=false'}`, settings),
  ...
  getVoiceSettings: ({ revealApiKey = false }: { revealApiKey?: boolean } = {}) =>
    get(`/api/settings/voice-settings${revealApiKey ? '?revealApiKey=true' : ''}`),
  updateVoiceSettings: (voice: unknown) => put('/api/settings/voice-settings', voice),
...
export async function synthesizeVoice(text: string, signal: AbortSignal): Promise<Response> {
  ...
  if (config.baseUrl.trim()) {
    // Same as transcription: a direct call to the user's own backend has to
    // carry the key itself, so fetch it lazily when this browser lacks it.
    const apiKey = config.apiKey || await revealVoiceApiKey();
