...
test('an auth error on a 401 ends the session', async () => {
  localStorage.setItem('auth-token', liveToken());
  let expiries = 0;
  const onExpired = () => {
    expiries += 1;
  ...
  window.addEventListener(AUTH_SESSION_EXPIRED_EVENT, onExpired);
  // Fork: the status matters. The server also sets X-Auth-Error on requests
  // that carried no token, so the header alone is not a rejection of ours.
...
test('an auth error without a 401 leaves a valid session alone', async () => {
  // The regression this guards: a 5xx or a proxy error page carrying the
  // header used to wipe a perfectly good token, logging the user out on
  // every page load.
  const token = liveToken();
  localStorage.setItem('auth-token', token);
  let expiries = 0;
  ...
  window.addEventListener(AUTH_SESSION_EXPIRED_EVENT, onExpired);
  respondWith({ 'X-Auth-Error': 'invalid' }, 502);
...
test('a 401 without a stored token cannot expire a session', async () => {
  // An unauthenticated startup call must not clear state it never owned.
  let expiries = 0;
  const onExpired = () => {
    expiries += 1;
  };
